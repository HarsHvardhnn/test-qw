...
import { X, Users, Building2, Shield, User, Search } from 'lucide-react';
...
interface Participant {
  id: string;
  name: string;
  role: 'contractor' | 'financial' | 'insurance' | 'inspector' | 'realtor' | 'investor';
  company: string;
  avatar?: string;
}
...
export const CreateConversationModal: React.FC<CreateConversationModalProps> = ({
  ...
  const fetchCustomerContractors = async () => {
    try {
      const response = await axiosInstance.get("/quote/v2/customer/contractors");
      return response.data; // Returns an array of contractors
    } catch (error) {
  ...
  // Sample participants data including the contractor
  const [availableParticipants,setActiveParticipants]= useState([
    {
      id: 'contractor1',
      name: contractorName,
      role: 'contractor',
  ...
  useEffect(() => {
    const getCustomers = async () => {
      try {
        const res = await fetchCustomerContractors();
        
        setActiveParticipants(res); // Set state with fetched data
      } catch (error) {
        console.error("Error fetching customers:", error);
      }
    };

    getCustomers(); // Call the async function
  }, []);
  console.log('active',availableParticipants)
        ...
        <div className="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          ...
          <div className="sm:flex sm:items-start">
            <div className="w-full">
              ...
              {/* Search Input - Removed search icon */}
              <div className="mb-4">
              ...
              {/* Participants List - Updated max-height */}
